...
public class WitEventAHandler
{
    ...
    /// restart-recovery).
    /// </summary>
    public async Task Handle(WitEventA message, Envelope envelope, RunMetrics metrics, ILogger<WitEventAHandler> logger)
    {
        if (string.Equals(message.Message__c, "poison", StringComparison.OrdinalIgnoreCase))
        {
            logger.LogWarning("Poison WitEventA received (ReplayId {ReplayId}, attempt {Attempt}) — throwing.",
        ...
        {
            logger.LogInformation("Slow WitEventA received (ReplayId {ReplayId}) — delaying 30s.", message.ReplayId);
            await Task.Delay(TimeSpan.FromSeconds(30));
        }

        // Recorded only on a successful pass so poison retries don't inflate the run's handled ledger.
        metrics.RecordHandled(nameof(WitEventA), message.ReplayId);
        logger.LogInformation(
            "Handled WitEventA from {Source} — ReplayId {ReplayId}, CreatedById {CreatedById}, CreatedDate {CreatedDate}",
